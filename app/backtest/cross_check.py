...
class CrossCheckRow:
    ...
    real_mark_result: str | None
    repriced: bool
    agree: bool | None  # results match (win/loss/scratch)
    sign_flip: bool  # recorded win vs real-mark loss, or vice versa
...
def cross_check_trade(
    ...
    if not t.long_id or not t.short_id:
        return CrossCheckRow(
            t.ref, t.symbol, t.strategy, t.recorded_net_pnl, None, None,
            recorded_result, None, False, None, False, None,
            note="not a two-leg vertical / missing legs",
        )
    ...
    if entry is None:
        return CrossCheckRow(
            t.ref, t.symbol, t.strategy, t.recorded_net_pnl, None, None,
            recorded_result, None, False, None, False, None,
            note="no aligned real marks on/before the entry date",
        )
    ...
    if exit_ is None:
        return CrossCheckRow(
            t.ref, t.symbol, t.strategy, t.recorded_net_pnl, None, None,
            recorded_result, None, False, None, False, None,
            note="no aligned real marks between entry and resolution date",
        )
    xl, xs, _ = exit_
    ...
    if not k1.fillable or not k05.fillable:
        return CrossCheckRow(
            t.ref, t.symbol, t.strategy, t.recorded_net_pnl, None, None,
            recorded_result, None, False, None, False, None,
            note="legs not fillable on entry/exit day",
        )
    rm_result = _result(k1.net_pnl_usd)
    agree = (recorded_result is not None and rm_result == recorded_result)
    sign_flip = (
        {recorded_result, rm_result} == {"win", "loss"}
    )
    gap = (
        round(t.recorded_net_pnl - k1.net_pnl_usd, 2)
        if t.recorded_net_pnl is not None else None
    )
    return CrossCheckRow(
        t.ref, t.symbol, t.strategy, t.recorded_net_pnl,
        round(k1.net_pnl_usd, 2), round(k05.net_pnl_usd, 2),
        recorded_result, rm_result, True, agree, sign_flip, gap,
    )
...
def summarize(rows: list[CrossCheckRow]) -> CrossCheckSummary:
    ...
    flips = [f"{r.ref} ({r.symbol}): ledger {r.recorded_result} ${r.recorded_pnl} "
             f"vs real-mark {r.real_mark_result} ${r.real_mark_pnl_k1}"
